...
public:
    ...
    const char* FindCRLF() const{   //  找到是否读到CRLF
    ...
    void Append(const char* message){     // 发送时装状态码、状态信息啥的
    ...
    void RetrieveAll() { //读完或写完所有数据后，改变索引
    ...
    int writeablebytes() const {return buffer_.size()-write_index_;}
